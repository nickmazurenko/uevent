...
export default class EventFrontService {
    ...
    createHandleFormSubmit(
        ...
        const handleFormSubmit = async (event: FormEvent<HTMLFormElement>) => {
            ...
            eventForm.append(
                "cost[amount]",
                this.eventData.cost.amount.toString()
            );
            ...
            eventForm.append(
                "duration",
                (
                    new Date(this.eventData.endAt).getTime() -
                    new Date(this.eventData.startAt).getTime()
                ).toString()
            );
            eventForm.append("tickets", this.eventData.tickets.toString());
            eventForm.append(
                "location",
                JSON.stringify(this.eventData.location)
            );
            ...
            if (response.ok) {
                ...
                if (callback) callback(data.data.ticketViewId);

                console.log(`${type}d event`, data);
            } else {
                console.error(
                    `Failed to ${type} event:`,
                    response.status,
                    response.statusText
                );
            }
    ...
    createHandleEventDataChange() {
        ...
        const handleEventDataChange = (
            event:
                ...
                | { target: { id: string; value: Tag[] | File[] | number } }
        ) => {
            if (event.target.id === EventFrontService.eventImageId) {
                ...
                this.setEventData({
                    ...eventData,
                    [EventFrontService.eventImageId]: images,
                });
